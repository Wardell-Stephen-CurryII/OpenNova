...
OpenNova v0.4.0 是一个从零开始构建的轻量级 Python CLI AI 编码助手。
...
- **Textual TUI**：分屏聊天界面，支持流式输出、会话恢复和 slash 命令
...
0.4.0 版本强化了计划、Skills、安全和 TUI 工作台体验：
- **双栏工作台 TUI**：左侧对话，右侧更宽的 Tools / Plan / Todos 面板，并支持快捷键切换
- **Plan mode 升级**：计划 markdown 持久化、每步执行前刷新、状态实时回写和 todo 镜像
- **Skills 升级**：命名空间发现、参数替换、hooks、paths 条件激活和排序
- **安全增强**：参数级权限、命令/网络/MCP 策略、secret 脱敏、审计日志和进程级沙箱
- **会话管理**：`/resume <id>`、`/sessions` — 对话持久化到 JSONL 文件
- **上下文压缩**：当 token 使用率超过 55% 时，LLM 自动总结旧消息，保持长对话在预算内
- **Textual TUI**：结构化对话块、文本复制、历史导航、实时流式输出和右侧详情面板
...
```bash
# Textual TUI（默认交互界面，包含 Windows 中文输入法兼容处理）
uv run opennova
