...
- 插件 trust 命令统一：把 `/plugins trust <name>`、`/plugins untrust <name>` 接入共享 `handle_plugin_command()`，保持 REPL/TUI 一致。
...
- 在 `opennova.cli.plugin_commands` 中新增 trust/untrust 子命令，并让 REPL/TUI 复用共享 handler。
...
- Plugins：`handle_plugin_command("trust demo")` 和 `untrust demo` 更新 trust store；REPL/TUI 可继续列表展示。
